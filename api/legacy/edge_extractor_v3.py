...
def main():
    ...
    image = cv2.imread('boundary_ori/UESTC_huzhou_106.jpg')  
    segmentation = np.loadtxt("boundary_txt/UESTC_huzhou_106.txt", delimiter=',',dtype=int)  
    detection_boxes = np.loadtxt("labels/UESTC_huzhou_106.txt")
    ...
    # use to convert 3d IMAGE to 2d image gray scale
    ...
    for index, cp in enumerate(cnts):
        if len(cp) > 100:  # consider only edges with more than 100 cordinates, remove small edges
            ...
            # print(below_class_2,  np.concatenate((below_class_1, below_class_2), axis=0))
            ...
            if above_class == 4 or current_class_anchor == 4:
                continue
                ...
                # print(current_class,above_class, below_class)
    ...
    cv2.imwrite("display_results.jpg", nump_img)
    np.savetxt("image_edges.txt", relabel_edge[:, :, 0].astype(int), fmt='%i')
    cv2.waitKey()
